...
public class HttpHandler extends ChannelInboundHandlerAdapter {
    ...
    //channelReadComplete表示读完了最后一行数据后干什么
    @Override
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (uri.contains("/test")){
            //当前请求中有/test请求时执行
            handlerTest(fullHttpRequest,ctx,"hello wmj!");
        }else {
    ...
    private void handlerTest(FullHttpRequest fullHttpRequest,ChannelHandlerContext ctx,String body){
        ...
        try {
            String value = body;
            response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, OK, Unpooled.wrappedBuffer(value.getBytes(StandardCharsets.UTF_8)));
